...
def fit_one_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    if (epoch+1)%100 == 0:
        ...
        torch.save(model.state_dict(), 'logs/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth'%((epoch+1),total_loss/(epoch_size+1),val_loss/(epoch_size_val+1)),_use_new_zipfile_serialization=False)
...
if __name__ == "__main__":
    ...
    if from_pretrained:
        model_path = "model_data/pretrained.pth"
        # 加快模型训练的效率
